...
function renderizzazione(richiesta, scriptId, divTarget) {
    ...
    document.querySelector(divTarget).innerHTML = rendered;
    if (scriptId === 'templateUno') { bindeventsUno();};
    if (scriptId === 'templateDue') { bindeventsDue();};
}

function settingInizialeUno() {
	//primo elemento e aggiungere classe selected
    document.querySelector('.giornoEdata').classList.add('selected');
    document.querySelector('.mattinaEpomeriggio').classList.add('selected');
    document.querySelector('.mattinaEpomeriggio:first-child svg').classList.add('noSvg');
    document.querySelector('.mattinaEpomeriggio:nth-child(2) svg').classList.add('bi-sun');
    document.querySelector('.mattinaEpomeriggio:last-child svg').classList.add('bi-sunset');
    document.querySelector('.orario').classList.add('nonDisponibile');
    document.querySelector('.orario:last-child').classList.add('nonDisponibile');
    for (let i = 1; i < document.querySelectorAll('.orario').length -1 ; i++) {
        document.querySelectorAll('.orario')[i].classList.add('disponibile');
    }
    carosello();
}
...
function bindeventsUno() {
	settingInizialeUno();
	//mettermi in ascolto su eventuali elementi html
...
function settingInizialeDue() {
	//aggiungere classi tipoDiSacchetti
    document.querySelectorAll('.bustePerSpesa > div:nth-child(2) .contatore span').forEach(function(item){item.classList.add('termiche');});
    document.querySelectorAll('.bustePerSpesa > div:nth-child(3) .contatore span').forEach(function(item){item.classList.add('bio');});
    document.querySelectorAll('.bustePerSpesa > div:last-child .contatore span').forEach(function(item){item.classList.add('riuso');});
}

function bindeventsDue() {
	settingInizialeDue();
...
function chiamatajaxDue() {
    fetch('src/json/stepDue.json').then(function (response) {
        return response.json();
      })
      .then(function (data) {
        renderizzazione(data, 'templateDue', '.stepDueInCorso');
      });
}

function chiamatajaxUno() {
    fetch('src/json/stepUno.json').then(function (response) {
        return response.json();
      })
      .then(function (data) {
        renderizzazione(data, 'templateUno', '.stepUnoInCorso');
      });
}
